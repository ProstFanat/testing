...
public class AclUserInGroupDAO {
    ...
    public ArrayList<String> getAclUserInGroupList(String orgId) throws SQLException {
        ACLUserInGroup aclUserInGroup = null;
        ArrayList<String> aclUsersInGroup = new ArrayList<>();

        Connection connection = DBConnection.getConnection(db, user, pass);
        String sql = "SELECT DISTINCT auig.*, aug.*, aug.user_group AS group_name FROM acl_user_in_group auig "
                + "JOIN acl_user_group aug ON aug.group_id = auig.group_id WHERE "
                + "auig.user_id IN (SELECT user_id from public.acl_user WHERE organization_id = " + orgId + ") ORDER BY group_name";
